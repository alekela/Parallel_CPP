...
int main(int argc, char* argv[]) {
	...
	filename += "_t_" + std::to_string(threads);
	...
	double manytime = MPI_Wtime();
	while (t_start < t_end) {
		# pragma omp parallel for collapse(2) private(tmp_Re_part)
		for (int i = fict; i < stop_y - start_y + fict; i++) {
			for (int j = fict; j < stop_x - start_x + fict; j++) {
		...
		// if (iter % iterwrite == 0 || t_start == t_end) write_data_parallel(u, v, t_start, Re, dx, dy, start_x, stop_x, start_y, stop_y, fict, rank_x, rank_y, filename);
		iter++;
	}
	if (rank == 0) {
